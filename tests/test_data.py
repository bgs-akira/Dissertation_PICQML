...
def test_load_save_roundtrip(tmp_path: Path):
    ...
    assert torch.allclose(loaded.V, ds.V)
...
def test_train_test_split_sizes_and_disjoint():
    ...
    assert len(test) == 5
    train_V = {tuple(v.tolist()) for v in train.V}
    test_V = {tuple(v.tolist()) for v in test.V}
...
def test_make_synthetic_dataset_shape_and_normalisation():
    ...
    ds = make_synthetic_dataset(model, n_samples=8, v_max=10.0, seed=0)
    assert len(ds) == 8
    assert ds.V.shape == (8, model.n_PS)
    assert ds.port.shape == (8,)
    assert ds.p.shape == (8, model.m)
    assert (ds.V >= 0).all() and (ds.V < 10.0).all()
